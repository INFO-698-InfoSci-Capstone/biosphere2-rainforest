...
author: "Sai Laasya Gorantla and Monica Tejaswi Kommareddy"
date: "2025-02-13"
...
-> We analyzed the completeness of CO₂ data for Zone 3 in 2019 by converting timestamps to datetime format, filtering for 2019 records, and counting the number of 15-minute readings per day. 
-> Since a complete day should have 96 readings, we identified 20 days with fewer readings, indicating missing data. Most of these days had slight gaps (e.g., 94 or 95 readings), but a few showed substantial missing values (e.g., only 69 readings), particularly during late September to early October. 
-> This analysis was crucial for ensuring data quality, as incomplete records could impact downstream calculations like mole differences and flux estimates.
...
-> This code refines the missing data analysis by identifying which hours within previously flagged dates had incomplete readings. 
-> Since each hour should have 4 readings (one every 15 minutes), the code groups data by date and hour, counts readings, and flags hours with fewer than 4. 
-> This makes it easier to pinpoint specific time gaps, avoiding the need to manually inspect entire days.
```{r}
...
```
##2020.
-> During the validation of 2019 CO₂ data from Zone 3, several days were found to have missing 15-minute readings and also the entries ranged between 13 and 17 minutes instead of the expected 15.
-> To maintain a complete and consistent time series, we identified the exact missing timestamps and imputed them using the annual mean CO₂ value (444.04 ppm). -> These values were clearly flagged to distinguish them from actual sensor data. This approach preserved the dataset’s structure, minimized bias, and ensured transparency for further analysis.
```{r}

# mean CO2 value for 2019
mean_value_2019 <- 444.0441

# List of missing timestamps (already POSIXct)
...
# Create the missing rows with correct data types and lengths
missing_rows <- data.frame(
  ID = rep("0000", length(missing_times)),
  TIMESTAMP = missing_times,
  VALUE = rep(mean_value_2019, length(missing_times)),
  TRENDFLAGS = rep(as.integer(0), length(missing_times)),
  STATUS = rep(as.integer(0), length(missing_times)),
  TRENDFLAGS_TAG = rep(" { }", length(missing_times)),
  STATUS_TAG = rep(" {ok}", length(missing_times)),
  stringsAsFactors = FALSE
)

# Combine with original dataset and sort by TIMESTAMP
co2_zone3_2019_new <- co2_zone3_2019 %>%
  bind_rows(missing_rows) %>%
  arrange(TIMESTAMP)

# Check the first rows
head(co2_zone3_2019_new)

# check for duplicated timestamps
#sum(duplicated(co2_zone3_2019_new$TIMESTAMP))
...
### Visualizing Daily Carbondioxide Concentration Trends in 2019
```{r}
# Plot
...
#outlier detection for 2019 uding KNN
```{r}
library(FNN)
library(ggplot2)
library(dplyr)

data_knn <- as.matrix(co2_zone3_2019_new$VALUE)
knn_dist <- kNNdist(data_knn, k = 6)
threshold <- quantile(knn_dist, 0.95)
co2_zone3_2019_new$Is_Outlier <- knn_dist > threshold

ggplot(co2_zone3_2019_new, aes(TIMESTAMP, VALUE, color = Is_Outlier, shape = Is_Outlier)) +
  geom_point(size = 0.75) +
  scale_color_manual(values = c("FALSE" = "pink", "TRUE" = "red")) +
  scale_shape_manual(values = c(16, 17)) +
  labs(title = "KNN Outlier Detection on CO2 Data 2019", color = "Outlier", shape = "Outlier") +
  theme_minimal()
...
```{r}
# Filter humidity data to include only records from 2019
ne_humid_2019 <- ne_humidity |> filter(year(DateTime) == 2019)

# Count the number of 15-minute interval readings per day
readings_per_day_humid_2019 <- ne_humid_2019 |> 
  group_by(Date = date(DateTime)) |> 
  summarize(Readings = n())

# Identify days in 2019 with missing or extra readings (not equal to 96)
missing_day_humid_2019 <- readings_per_day_humid_2019 |> 
  filter(Readings != 96)

cat("Dates with incomplete or irregular readings in 2019:\n")
...
```
#checking for null values in 2019 data across rows and columns
```{r}
## Filtering out the '2019' data from temperature dataframe
ne_temp_2019 <- ne_temperature |> filter(year(DateTime) == 2019)

## Counting the number of readings per day (15-min intervals → 96 per day)
readings_per_day_temp_2019 <- ne_temp_2019 |>
  group_by(Date = date(DateTime)) |>
  summarize(Readings = n())

## Identify days with missing readings
missing_day_temp_2019 <- readings_per_day_temp_2019 |>
  filter(Readings != 96)

cat("Missing dates on NE Temperature (2019):\n")
print(missing_day_temp_2019)
...
```{r}
# Select columns starting with "T_NE"
temp_columns <- grep("T_NE", names(ne_temp_2019), value = TRUE)

# Subset the data using the selected column names
temp_data <- ne_temp_2019[, temp_columns]

# Calculate the mean of those columns
ne_temp_mean_2019 <- colMeans(temp_data)

# Print the result
cat("Mean of the columns in the temperature dataframe")
print(ne_temp_mean_2019)
```
...
```{r}
# Extract 2019 Temperature Columns Starting with 'T_NE' 
temp_vars_2019 <- grep("^T_NE", names(ne_temp_2019), value = TRUE)
temp_data_2019 <- ne_temp_2019[temp_vars_2019]

#  Custom Outlier Detection Function Using KNN 
identify_knn_outliers <- function(vec) {
  knn_result <- knn.dist(as.matrix(vec), k = 6)
  local_quantile <- apply(knn_result, 1, function(row) quantile(row, 0.95))
  threshold <- quantile(local_quantile, 0.95)
  flagged <- local_quantile > threshold
  return(flagged)
}

# Loop Through Columns and Store Outlier Flags 
temp_outlier_flags <- lapply(temp_data_2019, identify_knn_outliers)
names(temp_outlier_flags) <- temp_vars_2019

#  Visualize Each Column with Outliers Highlighted 
for (temp_col in temp_vars_2019) {
  df_plot <- data.frame(
    Record = seq_along(temp_data_2019[[temp_col]]),
    Temp = temp_data_2019[[temp_col]],
    Outlier = temp_outlier_flags[[temp_col]]
  )
  
  plot_obj <- ggplot(df_plot, aes(x = Record, y = Temp)) +
    geom_point(aes(color = Outlier), shape = 21, size = 0.75, stroke = 0.6) +
    scale_color_manual(
      values = c("FALSE" = "#1b9e77", "TRUE" = "#d95f02"),
      labels = c("Normal", "Outlier")
    ) +
    labs(
      title = paste("Temperature Outlier Detection -", temp_col, " (2019)"),
      x = "Record Index",
      y = "Temperature Reading",
      color = "Category"
    ) +
    ...
    theme(
      ...
      panel.border = element_rect(color = "#aaaaaa", fill = NA)
    )
...
#Loading the file
temperature2 <- read.csv('data/S_Tower_Temperature.csv', skip = 4, header = FALSE)
...
#loading the file
temperature4 <- read.csv('data/Mtn_Tower_Temperature.csv', skip = 4, header = FALSE)
...
#Loading the file
humidity4 <- read.csv('data/Mtn_Tower_Relative humidity.csv', skip = 4, header = FALSE )

#Copying the dataframe
mtn_humid <- data.frame(humidity4)


# Taking off the null column 
...
#2019
-> The function filters each dataset to include only data from 2019.

-> It counts the number of 15-minute readings per day to check data completeness.

-> It identifies and prints days where readings are missing or incomplete (not exactly 96 readings).

-> The function returns that both NE humid and temp datasets for 2019 have some missing values which will be addressed lateron. 
```{r}
process_data <- function(tower, name) {
  tower_2019 <- tower |> filter(year(DateTime) == 2019)
  readings_per_day <- tower_2019 |> group_by(Date = date(DateTime)) |> summarize(Readings = n())
  missing_days <- readings_per_day |> filter(Readings != 96)
...
# Applying the function to all datasets
nw_humid_2019 <- process_data(nw_humid, "NW Humidity")
nw_temp_2019 <- process_data(nw_temp, "NW Temperature")
mtn_humid_2019 <- process_data(mtn_humid, "Mountain Humidity")
mtn_temp_2019 <- process_data(mtn_temp, "Mountain Temperature")
s_humid_2019 <- process_data(s_humid, "Southern Humidity")
s_temp_2019 <- process_data(s_temp, "Southern Temperature")

```
#calculating the mean for imputing missing values. 
```{r}
# Define the target columns
target_columns <- c("TRF_NWTower_100_HMP45", 
                    "TRF_NWTower_300_HMP45", 
                    "TRF_NWTower_700_HMP45", 
                    "TRF_NWTower_1300_HMP45")

# Calculate means for NW Humidity
nw_humid_100 <- mean(nw_humid_2019$TRF_NWTower_100_HMP45, na.rm = TRUE)
nw_humid_300 <- mean(nw_humid_2019$TRF_NWTower_300_HMP45, na.rm = TRUE)
nw_humid_700 <- mean(nw_humid_2019$TRF_NWTower_700_HMP45, na.rm = TRUE)
nw_humid_1300 <- mean(nw_humid_2019$TRF_NWTower_1300_HMP45, na.rm = TRUE)

# Calculate means for NW Temperature
nw_temp_100 <- mean(nw_temp_2019$TRF_NWTower_100_HMP45, na.rm = TRUE)
nw_temp_300 <- mean(nw_temp_2019$TRF_NWTower_300_HMP45, na.rm = TRUE)
nw_temp_700 <- mean(nw_temp_2019$TRF_NWTower_700_HMP45, na.rm = TRUE)
nw_temp_1300 <- mean(nw_temp_2019$TRF_NWTower_1300_HMP45, na.rm = TRUE)

```


-> imputation is done for both humidity and temprature 

```{r}
# Convert DateTime column to POSIXct
nw_humid_2019$DateTime <- as.POSIXct(nw_humid_2019$DateTime, format = "%Y/%m/%d %H:%M")
# List of missing timestamps
missing_timestamps <- as.POSIXct(c(
  "2019/09/03 16:15", "2019/09/03 16:30", "2019/09/03 16:45", "2019/09/03 17:00",
  "2019/09/03 17:15", "2019/09/03 17:30", "2019/09/03 17:45", "2019/09/03 18:00",
  "2019/09/03 18:15", "2019/09/03 18:30", "2019/09/03 18:45", "2019/09/03 19:00",
  "2019/09/03 19:15", "2019/09/03 19:30", "2019/09/03 19:45", "2019/09/06 11:15",
  "2019/09/06 11:30", "2019/09/06 11:45", "2019/09/15 02:45", "2019/09/15 03:00",
  "2019/09/15 03:15", "2019/09/15 03:30", "2019/09/15 03:45", "2019/09/15 04:00",
  "2019/09/16 09:15", "2019/09/16 09:30", "2019/09/16 09:45", "2019/09/16 10:00",
  "2019/09/16 10:15", "2019/09/16 10:30", "2019/09/16 10:45", "2019/09/16 11:00",
  "2019/09/16 11:15", "2019/09/16 11:30", "2019/09/16 11:45", "2019/09/16 12:00",
  "2019/09/16 12:15", "2019/09/16 12:30", "2019/09/16 12:45", "2019/10/13 22:15"
))

# Create the missing rows
missing_rows <- data.frame(
  DateTime = missing_timestamps,
  TRF_NWTower_100_HMP45 = nw_humid_100,
  TRF_NWTower_300_HMP45 = nw_humid_300,
  TRF_NWTower_700_HMP45 = nw_humid_700,
  TRF_NWTower_1300_HMP45 = nw_humid_1300
)

# Add them to the original dataset and sort


nw_humid_2019_new <- nw_humid_2019 %>%
  bind_rows(missing_rows) %>%
  arrange(DateTime)

print(nw_humid_2019_new)
```


```{r}
# Convert DateTime column to POSIXct
nw_temp_2019$DateTime <- as.POSIXct(nw_temp_2019$DateTime, format = "%Y/%m/%d %H:%M")

# Define the list of missing timestamps
missing_timestamps <- as.POSIXct(c(
  "2019/09/03 16:15", "2019/09/03 16:30", "2019/09/03 16:45", "2019/09/03 17:00",
  "2019/09/03 17:15", "2019/09/03 17:30", "2019/09/03 17:45", "2019/09/03 18:00",
  "2019/09/03 18:15", "2019/09/03 18:30", "2019/09/03 18:45", "2019/09/03 19:00",
...
# Replace these with the actual mean values from your temp dataset
nw_temp_100  <- mean(nw_temp_2019$TRF_NWTower_100_HMP45, na.rm = TRUE)
nw_temp_300  <- mean(nw_temp_2019$TRF_NWTower_300_HMP45, na.rm = TRUE)
nw_temp_700  <- mean(nw_temp_2019$TRF_NWTower_700_HMP45, na.rm = TRUE)
nw_temp_1300 <- mean(nw_temp_2019$TRF_NWTower_1300_HMP45, na.rm = TRUE)

# Create the missing rows with those mean values
missing_rows_temp <- data.frame(
  DateTime = missing_timestamps,
  TRF_NWTower_100_HMP45  = nw_temp_100,
  TRF_NWTower_300_HMP45  = nw_temp_300,
  TRF_NWTower_700_HMP45  = nw_temp_700,
  TRF_NWTower_1300_HMP45 = nw_temp_1300
)

#  Combine and sort the dataset


nw_temp_2019_new <- nw_temp_2019 %>%
  bind_rows(missing_rows_temp) %>%
  arrange(DateTime)

print(nw_temp_2019_new)
```
...
```{r}
#Temperature: Applied elevation-specific correction formulas to raw sensor data
nw_temp_2019_new <- nw_temp_2019_new %>%
  mutate(
    T_NW_100_Corr  = TRF_NWTower_100_HMP45 * 0.9922 + 0.6,
    T_NW_300_Corr  = TRF_NWTower_300_HMP45 * 1.005 - 0.17,
    T_NW_700_Corr  = TRF_NWTower_700_HMP45 * 1.007 - 0.14,
...
#Relative Humidity: Corrected raw values with standardized transformation equation
nw_humid_2019_new <- nw_humid_2019_new %>%
  mutate(
    RH_NW_100_Corr  = TRF_NWTower_100_HMP45 * 0.9756 - 1.2,
    RH_NW_300_Corr  = TRF_NWTower_300_HMP45 * 0.9882 - 1.6,
    RH_NW_700_Corr  = TRF_NWTower_700_HMP45 * 0.9895 + 1.1,
...
```{r}
# Combine temperature and humidity data from Southern and NE sensors for 2019 by DateTime
zone1_data_2019 <- s_temp_2019 |>
  select(DateTime, T_S_100_Corr, T_S_300_Corr) %>%
  left_join(
    s_humid_2019 %>%
      select(DateTime, RH_S_100_Corr, RH_S_300_Corr),
...
# View result
print("Zone 1 Data (2019):")
print(head(zone1_data_2019))
...
# Convert DateTime columns to character for proper joining
nw_temp_2019_new$DateTime <- as.character(nw_temp_2019_new$DateTime)
nw_humid_2019_new$DateTime <- as.character(nw_humid_2019_new$DateTime)

# Combine temperature and humidity data from multiple sensors for 2019 by DateTime
zone2_data_2019 <- nw_temp_2019_new %>%
  select(DateTime, T_NW_100_Corr, T_NW_300_Corr, T_NW_700_Corr) %>%
  left_join(
    s_temp_2019 %>%
      select(DateTime, T_S_700_Corr, T_S_1300_Corr),
...
# View result
print("Zone 2 Data (2019):")
print(head(zone2_data_2019))
...
```{r}
# Convert DateTime columns to character
nw_temp_2019_new$DateTime <- as.character(nw_temp_2019_new$DateTime)
nw_humid_2019_new$DateTime <- as.character(nw_humid_2019_new$DateTime)

# Combine temperature and humidity data from multiple sensors for 2019 by DateTime
zone3_data_2019 <- s_temp_2019 %>%
  select(DateTime, T_S_2000_Corr) %>%
  left_join(
    nw_temp_2019_new %>%
      select(DateTime, T_NW_1300_Corr),
...
```
filtering out for 2019 and finding the missing values
```{r}
# Convert TIMESTAMP to datetime and filter for 2019
co2_zone1_2019 <- co2_zone1 %>%
  mutate(TIMESTAMP = as.POSIXct(TIMESTAMP, format = "%Y/%m/%d %H:%M:%S")) %>%
  filter(year(TIMESTAMP) == 2019)

co2_zone2_2019 <- co2_zone2 %>%
  mutate(TIMESTAMP = as.POSIXct(TIMESTAMP, format = "%Y/%m/%d %H:%M:%S")) %>%
  filter(year(TIMESTAMP) == 2019)

# Count readings per day (should be 96 for 15-minute intervals)
readings_per_day_z1_2019 <- co2_zone1_2019 %>%
  group_by(Date = date(TIMESTAMP)) %>%
  summarize(Readings = n(), .groups = "drop")

readings_per_day_z2_2019 <- co2_zone2_2019 %>%
  group_by(Date = date(TIMESTAMP)) %>%
  summarize(Readings = n(), .groups = "drop")

# Identify days with missing readings
missing_day_z1_2019 <- readings_per_day_z1_2019 %>% filter(Readings != 96)
missing_day_z2_2019 <- readings_per_day_z2_2019 %>% filter(Readings != 96)

# Print results
cat("Missing dates of zone-1 (2019):\n")
print(missing_day_z1_2019)

cat("Missing dates of zone-2 (2019):\n")
print(missing_day_z2_2019)
```

found no missing values in missing_day_z1_2019 but missing_day_z2_2019 had some. Handling missing values with the mean value sand not interpolation because some of them had continous missing values and also on a large scale. 
```{r}
# List of missing timestamps for Zone 2, 2019
missing_times <- as.POSIXct(c(
  "2019-09-21 18:00:00", "2019-09-21 18:15:00", "2019-09-23 00:45:00", "2019-09-23 01:00:00",
  "2019-09-24 04:30:00", "2019-09-24 04:45:00", "2019-09-24 05:00:00", "2019-09-24 05:15:00",
  "2019-09-24 05:30:00", "2019-09-24 05:45:00", "2019-09-24 13:00:00", "2019-09-24 13:15:00",
  ...
  "2019-10-05 22:30:00", "2019-10-05 23:45:00", "2019-10-06 03:00:00", "2019-10-06 22:15:00",
  "2019-10-06 22:30:00", "2019-10-07 15:45:00", "2019-10-07 17:00:00", "2019-10-07 19:15:00",
  "2019-10-07 21:15:00", "2019-10-09 01:45:00", "2019-10-09 13:00:00", "2019-10-09 17:15:00",
  "2019-10-09 23:30:00", "2019-10-10 08:45:00", "2019-10-10 12:00:00", "2019-10-10 16:15:00",
  "2019-10-10 18:30:00", "2019-10-11 11:15:00", "2019-10-11 11:30:00", "2019-11-25 02:15:00",
  "2019-11-25 02:30:00", "2019-11-25 02:45:00", "2019-11-25 03:00:00", "2019-11-25 03:15:00",
  "2019-11-25 03:30:00", "2019-11-25 03:45:00", "2019-11-25 04:00:00", "2019-11-25 04:15:00",
  "2019-11-25 04:30:00", "2019-11-25 04:45:00", "2019-11-25 05:00:00", "2019-11-25 05:15:00",
  "2019-11-25 05:30:00", "2019-11-25 05:45:00", "2019-11-25 06:00:00", "2019-11-25 06:15:00",
  "2019-11-25 06:30:00", "2019-11-25 06:45:00", "2019-11-25 07:00:00", "2019-11-25 07:15:00",
  "2019-11-25 07:30:00", "2019-11-25 07:45:00", "2019-11-25 08:00:00", "2019-11-25 08:15:00",
  "2019-11-25 08:30:00", "2019-11-25 08:45:00", "2019-12-13 02:00:00", "2019-12-24 03:30:00"
))

# Compute the mean value from the original dataset (excluding NA if any)
mean_value_2019 <- mean(co2_zone2_2019$VALUE, na.rm = TRUE)

# Create the missing rows using the mean
missing_rows <- data.frame(
  ID = "0000",
  TIMESTAMP = missing_times,
...
co2_zone2_2019_new <- co2_zone2_2019 %>%
  bind_rows(missing_rows) %>%
  arrange(TIMESTAMP)
head(co2_zone2_2019_new)
...
```
Avg CO2 concentration across zone wise 
```{r}
# Calculate the daily average carbondioxide concentration for Zone 1 
avg_co2_zone1_2019 <- co2_zone1 |>
  mutate(Date = as.Date(TIMESTAMP),
         Year = year(Date)) |>
  filter(Year == 2019) |>                                           
  summarize(mean_co2 = mean(VALUE, na.rm = TRUE))|>              
...
# Calculate the daily average carbondioxide concentration for Zone 2 
avg_co2_zone2_2019 <- co2_zone2_2019_new |>
  mutate(Date = as.Date(TIMESTAMP),
         Year = year(Date)) |>
  filter(Year == 2019) |>                                           
  summarize(mean_co2 = mean(VALUE, na.rm = TRUE))|>              
...
# Calculate the daily average Carbondioxide concentration for Zone 3 
avg_co2_zone3_2019 <- co2_zone3_2019_new |>
  mutate(Date = as.Date(TIMESTAMP),
         Year = year(Date)) |>
  filter(Year == 2019) |>                                           
  summarize(mean_co2 = mean(VALUE, na.rm = TRUE))|>              
...
temp_zone1_2019 <- mean(zone1_data_2019$Zone_1_Temp_Avg) + 273.15
temp_zone2_2019 <- mean(zone2_data_2019$Zone_2_Temp_Avg) + 273.15
temp_zone3_2019 <- mean(zone3_data_2019$Zone_3_Temp_Avg) + 273.15
```
...
# Add daily CO2 moles to each zone's data for 2019
zone1_moles_2019 <- co2_moles(avg_co2_zone1_2019, temp_zone1_2019, vol_fracs[1])
zone2_moles_2019 <- co2_moles(avg_co2_zone2_2019, temp_zone2_2019, vol_fracs[2])
zone3_moles_2019 <- co2_moles(avg_co2_zone3_2019, temp_zone3_2019, vol_fracs[3])

print(paste("zone1 moles of co2:", zone1_moles_2019, 
            "zone-2 moles of co2:", zone2_moles_2019, 
            "zone-3 moles of co2:", zone3_moles_2019))
...
To analyze daily carbondioxide dynamics across the rainforest zones, we first process the raw sensor data to calculate daily average CO₂ concentrations for each vertical zone (1m, 13m, and 27m) throughout 2020 and 2019. This involves creating three separate datasets (co2_z1, co2_z2, co2_z3) where we: (1) convert timestamps to dates, (2) filter for 2020 measurements, and (3) compute daily mean CO₂ values while handling any missing data. These daily CO₂ averages are then joined to their corresponding zone datasets (zone1_data, zone2_data, zone3_data) which already contain the daily temperature and humidity measurements.
...
```{r}
# Step 1: Create daily CO2 average for 2019
co2_z1_2019 <- co2_zone1_2019 %>%
  mutate(Date = as.Date(TIMESTAMP), 
         Year = year(Date)) %>%
  filter(Year == 2019) %>%
  group_by(Date) %>%
  summarize(carbondioxide_z1 = mean(VALUE, na.rm = TRUE), .groups = "drop")

# Step 2: Join on 'Date'
zone1_data_2019 <- zone1_data_2019 %>%
  left_join(co2_z1_2019, by = "Date")

# Step 3: Check your dataframe
head(zone1_data_2019)
```


```{r}
# Step 1: Create daily CO2 average for 2019
co2_z2_2019 <- co2_zone2_2019_new %>%
  mutate(Date = as.Date(TIMESTAMP), 
         Year = year(Date)) %>%
  filter(Year == 2019) %>%
  group_by(Date) %>%
  summarize(carbondioxide_z2 = mean(VALUE, na.rm = TRUE), .groups = "drop")

# Step 2: Join on Date
zone2_data_2019 <- zone2_data_2019 %>%
  left_join(co2_z2_2019, by = "Date")

# Step 3: Check the resulting dataframe
head(zone2_data_2019)
```
```{r}
# Step 1: Create daily CO2 average for 2019 in zone 3
co2_z3_2019 <- co2_zone3_2019_new %>%
  mutate(Date = as.Date(TIMESTAMP), Year = year(Date)) %>%
  filter(Year == 2019) %>%
  group_by(Date) %>%
  summarize(carbondioxide_z3 = mean(VALUE, na.rm = TRUE), .groups = "drop")

# Step 2: Join on Date
zone3_data_2019 <- zone3_data_2019 %>%
  left_join(co2_z3_2019, by = "Date")

# Step 3: Check the result
head(zone3_data_2019)
```
...
```{r}
#Adding temp with 273K to convert it to kelvin
zone1_data_2019$Zone_1_Temp_Avg <- (zone1_data_2019$Zone_1_Temp_Avg) + 273.15
zone2_data_2019$Zone_2_Temp_Avg <- (zone2_data_2019$Zone_2_Temp_Avg) + 273.15
zone3_data_2019$Zone_3_Temp_Avg <- (zone3_data_2019$Zone_3_Temp_Avg) + 273.15
...
#Zone 1,2,3 difference of moles calculated between consective dates
zone1_data_2019$Zone_1_CO2_Diff <- c(NA, diff(zone1_data_2019$moles))
zone2_data_2019$Zone_2_CO2_Diff <- c(NA, diff(zone2_data_2019$moles))
zone3_data_2019$Zone_3_CO2_Diff <- c(NA, diff(zone3_data_2019$moles))
```
...
```{r}
zone1_minute_data_2019 <- co2_zone1_2019 %>%
  mutate(TIMESTAMP = as.POSIXct(TIMESTAMP, format = "%Y/%m/%d %H:%M:%S")) %>%
  filter(year(TIMESTAMP) == 2019)

zone2_minute_data_2019 <- co2_zone2_2019_new %>%
  mutate(TIMESTAMP = as.POSIXct(TIMESTAMP, format = "%Y/%m/%d %H:%M:%S")) %>%
  filter(year(TIMESTAMP) == 2019)

zone3_minute_data_2019 <- co2_zone3_2019_new %>%
  mutate(TIMESTAMP = as.POSIXct(TIMESTAMP, format = "%Y/%m/%d %H:%M:%S")) %>%
  filter(year(TIMESTAMP) == 2019)
```
...
# Prepare Zone 1 temp and humidity for 2019
zone1_temp_humid_2019 <- s_temp_2019 %>%
  select(DateTime, T_S_100_Corr, T_S_300_Corr) %>%
  left_join(
    ne_temp_2019 %>%
      select(DateTime, T_NE_100_Corr, T_NE_300_Corr),
...
# Prepare Zone 2 CO₂ minute data for 2019
zone2_minute_data_2019 <- co2_zone2 %>%
  mutate(TIMESTAMP = as.POSIXct(TIMESTAMP, format = "%Y/%m/%d %H:%M:%S")) %>%
  filter(year(TIMESTAMP) == 2019)
head(zone2_minute_data_2019)
...
```{r}
zone2_temp_humid_2019 <- nw_temp_2019_new %>%
  select(DateTime, T_NW_100_Corr, T_NW_300_Corr, T_NW_700_Corr) %>%
  left_join(
    s_temp_2019 %>% select(DateTime, T_S_700_Corr, T_S_1300_Corr),
    by = "DateTime"
  ) %>%
  left_join(
    ne_temp_2019 %>% select(DateTime, T_NE_700_Corr, T_NE_1300_Corr),
    by = "DateTime"
  ) %>%
  left_join(
    mtn_temp_2019 %>% select(DateTime, T_MTN_100_Corr, T_MTN_300_Corr),
    by = "DateTime"
  ) %>%
  left_join(
    nw_humid_2019_new %>% select(DateTime, RH_NW_100_Corr, RH_NW_300_Corr, RH_NW_700_Corr),
    by = "DateTime"
  ) %>%
  left_join(
    s_humid_2019 %>% select(DateTime, RH_S_700_Corr, RH_S_1300_Corr),
    by = "DateTime"
  ) %>%
  left_join(
    ne_humid_2019 %>% select(DateTime, RH_NE_700_Corr, RH_NE_1300_Corr),
    by = "DateTime"
  ) %>%
  left_join(
    mtn_humid_2019 %>% select(DateTime, RH_MTN_100_Corr, RH_MTN_300_Corr),
    by = "DateTime"
  ) %>%
  mutate(
    TIMESTAMP = as.POSIXct(DateTime, format = "%Y/%m/%d %H:%M"),
...
```{r}
zone3_temp_humid_2019 <- ne_temp_2019 %>% 
  select(DateTime, T_NE_2000_Corr) %>% 
  left_join(
    s_temp_2019 %>% select(DateTime, T_S_2000_Corr), by = "DateTime"
  ) %>%
  left_join(
    nw_temp_2019_new %>% select(DateTime, T_NW_1300_Corr), by = "DateTime"
  ) %>%
  left_join(
    mtn_temp_2019 %>% select(DateTime, T_MTN_700_Corr, T_MTN_1300_Corr), by = 'DateTime'
  ) %>%
  left_join(
    s_humid_2019 %>% select(DateTime, RH_S_2000_Corr), by = "DateTime"
  ) %>%
  left_join(
    nw_humid_2019_new %>% select(DateTime, RH_NW_1300_Corr), by = "DateTime"
  ) %>%
  left_join(
    ne_humid_2019 %>% select(DateTime, RH_NE_2000_Corr), by = "DateTime"
  ) %>%
  left_join(
    mtn_humid_2019 %>% select(DateTime, RH_MTN_700_Corr, RH_MTN_1300_Corr), by = "DateTime"
  ) %>%
  mutate(
    TIMESTAMP = as.POSIXct(DateTime, format= "%Y/%m/%d %H:%M"),
    Temp_Avg = rowMeans(select(., starts_with("T_")), na.rm = TRUE),
...
```{r}
zone1_minute_data_2019 <- zone1_temp_humid_2019 %>%
  left_join(zone1_minute_data_2019, by = "TIMESTAMP")

zone2_minute_data_2019 <- zone2_temp_humid_2019 %>%
  left_join(zone2_minute_data_2019, by = "TIMESTAMP")

zone3_minute_data_2019 <- zone3_temp_humid_2019 %>%
  left_join(zone3_minute_data_2019, by = "TIMESTAMP")
...
```{r}
zone1_minute_data_2019 <- zone1_minute_data_2019 |> mutate(Temp_Avg = Temp_Avg + 273.15)
zone2_minute_data_2019 <- zone2_minute_data_2019 |> mutate(Temp_Avg = Temp_Avg + 273.15)
zone3_minute_data_2019 <- zone3_minute_data_2019 |> mutate(Temp_Avg = Temp_Avg + 273.15)
...
```{r}
zone1_minute_data_2019 <- zone1_minute_data_2019 |> 
  mutate(moles = co2_moles(VALUE, Temp_Avg, vol_fracs[1]))

zone2_minute_data_2019 <- zone2_minute_data_2019 |> 
  mutate(moles = co2_moles(VALUE, Temp_Avg, vol_fracs[2]))

zone3_minute_data_2019 <- zone3_minute_data_2019 |> 
  mutate(moles = co2_moles(VALUE, Temp_Avg, vol_fracs[3]))
...
```{r}
zone1_minute_data_2019$diff_moles <- c(NA, diff(zone1_minute_data_2019$moles))
zone2_minute_data_2019$diff_moles <- c(NA, diff(zone2_minute_data_2019$moles))
zone3_minute_data_2019$diff_moles <- c(NA, diff(zone3_minute_data_2019$moles))
...
```
calculating mole diffrerences across all three zones for every 15 minutes for the 2019 year.
```{r}
# Select TIMESTAMP and diff_moles columns for each zone's 2019 data
zone1_min_sub_2019 <- zone1_minute_data_2019[, c("TIMESTAMP", "diff_moles")]
zone2_min_sub_2019 <- zone2_minute_data_2019[, c("TIMESTAMP", "diff_moles")]
zone3_min_sub_2019 <- zone3_minute_data_2019[, c("TIMESTAMP", "diff_moles")]

# Rename the diff_moles column to zone-specific names
colnames(zone1_min_sub_2019)[2] <- "diff_moles_z1"
colnames(zone2_min_sub_2019)[2] <- "diff_moles_z2"
colnames(zone3_min_sub_2019)[2] <- "diff_moles_z3"

# Merge the three dataframes on TIMESTAMP with a full outer join
merged_min_moles_2019 <- Reduce(function(x, y) merge(x, y, by = "TIMESTAMP", all = TRUE),
                               list(zone1_min_sub_2019, zone2_min_sub_2019, zone3_min_sub_2019))

# Sum the diff_moles values across all zones, ignoring NAs
merged_min_moles_2019$total_diff_moles <- rowSums(merged_min_moles_2019[, c("diff_moles_z1", "diff_moles_z2", "diff_moles_z3")], na.rm = TRUE)


```

Time Series Plot of Total CO₂ Mole Differences for 2019 data
```{r}

ggplot(merged_min_moles_2019, aes(x = TIMESTAMP, y = total_diff_moles)) +
  geom_line(color = "#C18CAF") +
  labs(
    title = "Time Series of Total CO2 Mole Differences (2019)",
    x = "Date",
    y = "Total Difference in Moles of CO2"
  ) +
  theme_minimal()
