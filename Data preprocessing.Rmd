...
readings_per_day <- co2_zone3 |> group_by(Date = date(TIMESTAMP)) %>% summarize(Readings = n())
...
### Visualizing Daily Crabondioxide Concentration Trends in 2020
...
ne_humidity_2020 <- ne_humidity |> filter(year(DateTime) == 2020)
                           
                           
##  Counting the number as the readings takes place for every 15 min
readings_per_day_humid <- ne_humidity_2020 |> group_by(Date = date(DateTime)) %>% summarize(Readings = n())
...
ne_humidity_2020 <- ne_humidity_2020 %>%
  mutate(
...
rh_columns <- grep("RH", names(ne_humidity_2020), value = TRUE)

# Subset the data using the selected column names
rh_data <- ne_humidity_2020[, rh_columns]
...
humidity_rh <- ne_humidity_2020[, rh_columns]
...
ne_temperature_2020 <- ne_temperature |> filter(year(DateTime) == 2020)
                           
                           
##  Counting the number as the readings takes place for every 15 min
readings_per_day_temp<- ne_temperature_2020 |> group_by(Date = date(DateTime)) %>% summarize(Readings = n())
...
ne_temperature_2020 <- ne_temperature_2020 %>%
  mutate(
...
t_columns <- grep("T_NE", names(ne_temperature_2020), value = TRUE)

# Subset the data using the selected column names
t_data <- ne_temperature_2020[, t_columns]
...
temp_columns <- grep("^T_NE", names(ne_temperature_2020), value = TRUE)

# Considering values of those columns
temp_df <- ne_temperature_2020[, temp_columns]
...
zone1_data <- s_temp_2020 %>%
  ...
  group_by(Date) %>%
  summarize(
    Zone_1_Temp_Avg = mean(c(s_temp_2020$T_S_100_Corr, 
                             s_temp_2020$T_S_300_Corr, 
                             temperature1$T_NE_100_Corr, 
                             temperature1$T_NE_300_Corr), na.rm = TRUE),
    
    Zone_1_RH_Avg = mean(c(s_humid_2020$RH_S_100_Corr,
                           s_humid_2020$RH_S_300_Corr, 
                           humidity1$RH_NE_100_Corr, 
                           humidity1$RH_NE_300_Corr), na.rm = TRUE)
  )
